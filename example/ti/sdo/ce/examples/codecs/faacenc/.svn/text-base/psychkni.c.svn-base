...
static void PsyCheckShort(PsyInfo * psyInfo)
{
  ...
  {
    int win;
    static double volb[16];
    static double vavg[13];
    double maxdif = 0.0;
			 ...
			 int num_cb_short, unsigned int numChannels)
{
  ...
  double _transBuff[2 * BLOCK_LEN_LONG];
  double _transBuffS[2 * BLOCK_LEN_SHORT];
...
{
  int win;
  double *transBuff = _transBuff;
  double *transBuffS = _transBuffS;
  psydata_t *psydata = psyInfo->data;
  psyfloat *tmp;
  int sfb;
